import React from 'react'
import "./develop.css"
function Develop({data}) {
  return (
    <div className='marg' style={{paddingTop:'40px'}}>
        <section  style={{width:"90%",margin:'0px auto',}}>

<div className='best-in-app'>
<h2 style={{color:'white',fontSize:"50px",paddingTop:"55px",fontWeight:700}} >We Develop Great  <span style={{color:'rgb(58, 242, 181)'}}> {data=="app"? "Mobile Apps" :"Websites"}  </span> <br /> For Great <span style={{color:'rgb(58, 242, 181)'}}>Brands</span> </h2>

<p style={{color:'white',fontSize:"20px",width:"74%",wordSpacing:"1px",letterSpacing:'0',fontWeight:100,marginTop:'10px'}}>For the past decade we have been creating high performance, feature-packed mobile applications for iOS & Android. As an expert in mobile application development services, Folio3 can create the right app that will meet all your business and industry needs.</p>


</div>
</section>
    </div>
  )
}

export default Develop